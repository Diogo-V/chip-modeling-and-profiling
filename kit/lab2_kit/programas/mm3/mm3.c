...
#define N 512
#define CACHE_LINE_SIZE 512 // TODO: update this value

#define SUB_MATRIX_SIZE (((CACHE_LINE_SIZE / sizeof(int16_t)) <= 0) ? 1 : (CACHE_LINE_SIZE / sizeof(int16_t)))
                ...
                for (size_t inner_i = 0; inner_i < SUB_MATRIX_SIZE; ++inner_i) {
                    for (size_t inner_k = 0; inner_k < SUB_MATRIX_SIZE; ++inner_k) {
                        for (size_t inner_j = 0; inner_j < SUB_MATRIX_SIZE; ++inner_j) {
                            res[i+ inner_i][j + inner_j] += factor1[i + inner_i][k + inner_k] * factor2[k + inner_k][j + inner_j];
                        }
                    }
                }
            }
